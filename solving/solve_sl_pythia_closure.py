...
def solve_event(lep, nu_true, qvis, qlost_true, pool, pool_labels):
    ...
    for b_had_idx, b_lep_idx in permutations(range(4), 2):
        ...
        H_py = bH1["py"] + bH2["py"]
        H_pz = bH1["pz"] + bH2["pz"]
        # Reconstructed Higgs mass for THIS candidate = invariant mass of
        # whichever two pool jets are NOT used as b_had/b_lep. Correct iff
        # the b_had/b_lep pairing itself was correct (pool has only 4
        # members, so getting b_had/b_lep right automatically leaves
        # {higgsb1, higgsb2} as the remainder) -- no separate flag needed,
        # "correct_pairing" already covers it.
...
def main():
    ...
    for i in range(n_do):
        ...
        out["higgs_mass_true"].append(inv_mass(
        ...
        if best is None:
            ...
            out["higgs_mass_reco"].append(np.nan)
        else:
            n_with_solution += 1
            ...
            out["higgs_mass_reco"].append(best["higgs_mass_reco"])
